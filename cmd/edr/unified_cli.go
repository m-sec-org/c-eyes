...
var eventlogExportFunc = eventlogscan.Export
...
func runEventlogCLI(args []string, global globalCLIOptions) int {
	...
	exported, err := eventlogExportFunc(context.Background(), parsed.Params)
	if err != nil {
		progress.Done()
		fmt.Fprintln(os.Stderr, err)
	...
	if err := emitEventlogOutput(exported, global.OutputPath); err != nil {
		fmt.Fprintln(os.Stderr, err)
		return 1
	}
	return 0
...
func attachRiskStreamingCallbacks(analyzer *riskanalysis.Analyzer, progress *terminalProgress, out *os.File) {
	if analyzer == nil || progress == nil {
		return
	}
	colorEnabled := supportsANSIColorOutput(out)
...
func analyzeRiskResults(opts riskOptions, overrideRecords []riskanalysis.ScanRecord, progress *terminalProgress) ([]riskanalysis.AnalysisResult, error) {
	...
	attachRiskStreamingCallbacks(&analyzer, progress, os.Stderr)
...
func emitEventlogOutput(exported *eventlogscan.ExportResult, outputPath string) error {
	return emitEventlogOutputWithPrompt(exported, outputPath, os.Stdout, os.Stderr, os.Stdin, defaultPermissionFallbackPrompt)
}

func emitEventlogOutputWithPrompt(exported *eventlogscan.ExportResult, outputPath string, stdout io.Writer, stderr io.Writer, stdin io.Reader, prompt permissionFallbackPrompt) error {
	if exported == nil {
		return fmt.Errorf("eventlog export result is nil")
	}
	if stdout == nil {
		stdout = io.Discard
	}
	...
	if strings.TrimSpace(resolvedOutputPath) == "" {
		return writeEventlogJSONToWriter(stdout, exported)
	}
	...
	case "json":
		...
		err = writeEventlogJSONFile(resolvedOutputPath, exported)
	case "csv":
		writtenPaths = []string{resolvedOutputPath}
		err = writeEventlogCSVFile(resolvedOutputPath, exported)
	case "xlsx":
		writtenPaths, err = writeEventlogXLSXFiles(resolvedOutputPath, exported)
	default:
	...
	if err != nil {
		if isPermissionDeniedError(err) {
			allowJSONFallback, promptErr := prompt(resolvedOutputPath, err, stdin, stderr)
			if promptErr != nil {
				return fmt.Errorf("output fallback prompt failed: %w", promptErr)
			}
			if allowJSONFallback {
				if fallbackErr := writeEventlogJSONToWriter(stdout, exported); fallbackErr != nil {
					return fmt.Errorf("output fallback failed: %w", fallbackErr)
				}
				return nil
			}
		}
...
func writeEventlogJSONFile(path string, exported *eventlogscan.ExportResult) error {
	file, err := os.Create(path)
	if err != nil {
		return err
	}
	defer func() { _ = file.Close() }()
	return writeEventlogJSONToWriter(file, exported)
}

func writeEventlogJSONToWriter(w io.Writer, exported *eventlogscan.ExportResult) error {
	if exported == nil {
		return fmt.Errorf("eventlog export result is nil")
	}
	...
	if _, err := fmt.Fprintf(writer, "  \"total\": %d,\n", exported.Total); err != nil {
		return err
	}
	if _, err := writer.WriteString("  \"rows\": [\n"); err != nil {
		return err
	}
	...
	for {
		...
		if _, err := writer.Write(rowBytes); err != nil {
			return err
		}
	}
	if !first {
...
func writeEventlogCSVFile(path string, exported *eventlogscan.ExportResult) error {
	headers, err := collectEventlogHeaders(exported)
	if err != nil {
		return err
	}
	...
	for {
		...
		record, err := eventlogRowToCSVRecord(row, headers)
		if err != nil {
			return err
		}
		if err := writer.Write(record); err != nil {
...
func writeEventlogXLSXFiles(path string, exported *eventlogscan.ExportResult) ([]string, error) {
	headers, err := collectEventlogHeaders(exported)
	if err != nil {
		return nil, err
	}
	if len(headers) > xlsxMaxColumns {
		return nil, fmt.Errorf("xlsx output exceeds column limit (%d > %d); use -o <file>.csv or -o <file>.json", len(headers), xlsxMaxColumns)
	}

	rowsPerFile := xlsxMaxRows - 1
	totalParts := 1
	if exported.Total > rowsPerFile {
		totalParts = (exported.Total + rowsPerFile - 1) / rowsPerFile
	}
	...
	writtenPaths := make([]string, 0, totalParts)
	remaining := exported.Total
	for part := 1; part <= totalParts; part++ {
		partRows := remaining
		if partRows > rowsPerFile {
			partRows = rowsPerFile
		}
		if partRows < 0 {
			partRows = 0
		}
		partPath := xlsxShardPath(path, part, totalParts > 1)
		if err := writeEventlogXLSXFilePart(partPath, iter, headers, partRows); err != nil {
			return nil, err
		}
		writtenPaths = append(writtenPaths, partPath)
		remaining -= partRows
...
func writeEventlogXLSXFilePart(path string, iter eventlogscan.RowIterator, headers []string, rows int) error {
	file := excelize.NewFile()
	sheet := "results"
	file.SetSheetName(file.GetSheetName(0), sheet)
	...
	for rowIdx := 0; rowIdx < rows; rowIdx++ {
		...
		flat, err := eventlogRowToFlatMap(row)
		if err != nil {
			return err
		}
		values := make([]any, len(headers))
		for i, header := range headers {
			values[i] = flat[header]
		}
...
func collectEventlogHeaders(exported *eventlogscan.ExportResult) ([]string, error) {
	if exported == nil {
	...
	return orderDisplayHeaders(headers), nil
}

func eventlogRowToCSVRecord(row eventlogscan.EventRow, headers []string) ([]string, error) {
	flat, err := eventlogRowToFlatMap(row)
	if err != nil {
		return nil, err
	}
...
func eventlogRowToFlatMap(row eventlogscan.EventRow) (map[string]string, error) {
	rowMap, err := anyToMap(row)
	if err != nil {
		return nil, err
	}
	return flattenRow(rowMap), nil
}
...
func eventlogUsage() {
	...
	fmt.Fprintln(os.Stderr, "    eventlog streams large result sets through bounded disk-backed export; pagination is disabled.")
}
